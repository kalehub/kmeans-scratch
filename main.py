...
def init_cluster(cen, data, k):
    ...
    while(True):
        ...
        print(f'cluster baru:\n{new_cluster}')
        itr+=1
        centroid = new_cluster
    
        if (new_cluster == centroid).all():
            result = data_with_cluster
            print('Nilai UTS | Nilai Tugas | Nilai UAS| Cluster |')
            print(result)
            print(f'clustering selesai pada iterasi ke-{itr}')
